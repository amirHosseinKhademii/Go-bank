...
//   • A `.env` at the repo root with at least DB_SOURCE, DB_TEST, REDIS_URL, JWT_SECRET. Any KAFKA_* / SERVICEBUS_* keys present are
...
const NAMESPACE = process.env.NAMESPACE ?? 'bank-app';
const SECRET_NAME = process.env.SECRET_NAME ?? 'bank-app-secrets';
const NAMESPACE_MANIFEST = resolve(REPO_ROOT, 'k8s/base/namespace.yaml');

const REQUIRED = ['DB_SOURCE'];
...
if (!existsSync(ENV_FILE)) {
  die(
    `No .env at ${ENV_FILE} — copy k8s/base/secrets.example.yaml or fill in your own.`,
  );
}
